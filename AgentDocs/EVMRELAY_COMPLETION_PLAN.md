...
These fields must not be treated as aliases. `nodes` are the preferred input for `eth_watch` and relay observation. `bootnodes` are discovery infrastructure and should not be assumed to speak ETH status or provide block/event data.
...
- Add Gnosis Chain coverage for the no-pre-cached-nodes path: `chain_enodes.json.gz.bootnodes` seeds discv4 discovery, and discovered peers feed the dial queue.
